<script lang="ts">
 ...
 $effect(()=>{const id=page.url.searchParams.get('person');if(id && id!==lastRequestedId && people.some(p=>p.id===id)){lastRequestedId=id;open(people.find(p=>p.id===id));}});
 ...
 function open(p?: Person) { confirmDelete=false; selectedId=p?.id ?? null; creating=!p; permissionMode=p?.modulePermissions===null || !p ? 'default':'custom'; }
...
<style>
 .people{max-width:1200px;margin:auto;color:var(--text-strong,#17212b)}header,.filters{display:flex;gap:1.5rem;justify-content:space-between;align-items:center;margin-bottom:1.5rem}h1{font-size:1.8rem;font-weight:750}h2{font-size:1.3rem;font-weight:700;margin-bottom:1rem}h3{font-weight:700;margin:1.5rem 0 .75rem}.eyebrow{text-transform:uppercase;font-size:.7rem;letter-spacing:.15em}p,.help{color:var(--text-muted,#667085)}.help{font-size:.8rem;margin:.5rem 0 1rem}.filters>label:first-child{flex:1;max-width:420px}.workspace{display:grid;align-items:start;grid-template-columns:minmax(220px,1fr) minmax(0,2fr);gap:1.5rem}.directory{overflow:hidden;align-self:start}.directory,.editor{background:white;border:1px solid #e4e7ec;border-radius:12px}.editor{padding:1.5rem;min-width:0}.removal{border-top:1px solid #e4e7ec;margin-top:1.5rem}.delete-actions{display:flex;gap:.75rem;flex-wrap:wrap}.secondary{background:white;color:#344054}.person{display:flex;flex-direction:column;gap:.3rem;width:100%;text-align:left;border:0;border-bottom:1px solid #eee;border-radius:0;background:white;color:inherit;padding:1rem}.person span{font-size:.8rem;color:#667085}.person.selected span{color:#475467}.person.selected{background:var(--accent-soft,#fff3e9)}.person:focus-visible{outline:2px solid var(--accent-text,#c2410c);outline-offset:-3px}.fields,.permissions{display:grid;grid-template-columns:1fr 1fr;gap:1rem}label{display:flex;flex-direction:column;gap:.4rem;font-size:.85rem;margin-bottom:.8rem}input,select{border:1px solid #ccd2da;border-radius:6px;padding:.65rem;min-width:0;background:white;color:#17212b}button{border:1px solid #d1d5db;border-radius:6px;padding:.65rem 1rem;background:var(--accent-text,#c2410c);color:white;font-weight:600}button:disabled,fieldset:disabled{opacity:.6}fieldset{border:0;padding:0;min-width:0}.profiles{display:flex;flex-wrap:wrap;gap:1rem;margin:1rem 0}.profiles label{flex-direction:row;align-items:center;text-transform:capitalize}.permissions label{text-transform:capitalize}.notice{background:#effaf4;padding:1rem;margin-bottom:1rem;border-radius:8px;overflow-wrap:anywhere}.error{background:#fff0f0;color:#9f1239}.empty{padding:1.5rem}.danger{background:white;color:#be123c;margin-top:.5rem}a{text-decoration:underline}@media(max-width:760px){.workspace,.fields,.permissions{grid-template-columns:1fr}.filters,header{align-items:stretch;flex-direction:column}.directory{max-height:280px;overflow:auto}}
</style>
